...
export default function ResidualMaterialTab({ user }: ResidualMaterialTabProps) {
  ...
  const uniqueColors = useMemo(() => {
    return Array.from(new Set(bunProperties.map(b => b.mau).filter(Boolean))).sort()
  }, [bunProperties])

  const uniqueDensities = useMemo(() => {
    return Array.from(new Set(bunProperties.map(b => b.density).filter(Boolean))).sort()
  }, [bunProperties])

  const uniqueHardness = useMemo(() => {
    return Array.from(new Set(bunProperties.map(b => b.do_cung).filter(Boolean))).sort()
  }, [bunProperties])

  const uniquePowders = useMemo(() => {
    return Array.from(new Set(bunProperties.map(b => b.bot).filter(Boolean))).sort()
  }, [bunProperties])

  const uniqueLengths = useMemo(() => {
    return Array.from(new Set(bunProperties.map(b => b.chieu_dai).filter(Boolean))).sort()
  }, [bunProperties])
