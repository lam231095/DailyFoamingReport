# ==============================================================
# sync-china-plan.ps1
# Đọc sheet "Master BCN" trong "Lam_BCN master 19.05.xlsx" và UPSERT lên Supabase
# Khi firm_plan trùng, dữ liệu mới sẽ ghi đè dữ liệu cũ (merge-duplicates)
# Chạy: powershell -ExecutionPolicy Bypass -File ".\scripts\sync-china-plan.ps1"
# ==============================================================
...
$EXCEL_FILE   = Join-Path $PROJECT_ROOT "Lam_BCN master 19.05.xlsx"
$SHEET_NAME   = "Master BCN"

# Layout cột (1-indexed) dựa theo phân tích dòng 4
$COL_NO_ORDER   = 2
$COL_FIRM_PLAN  = 4
$COL_BUN_CODE   = 5
$COL_PU_CODE    = 6
$COL_TEN_SP     = 7
$COL_SL_SHEET   = 10   # QTY Actual Delivered (Sheet)
$COL_SL_DO      = 15   # Qty (BUN) (Pur Qty)
$COL_SL_TACH    = 15   # Qty (BUN) (Pur Qty)
$COL_COMPLETION = 18   # ETD Bun China
$COL_DELIVERY   = 32   # ETD system (Fix)
...
Write-Host "  CHINA PLAN (BCN) → SUPABASE SYNC" -ForegroundColor Cyan
...
try {
    ...
    Write-Host "[1/3] Đang đọc sheet '$SHEET_NAME' ($nr rows)" -ForegroundColor Green
...
Write-Host ""
Write-Host "[2/3] Tổng cộng $totalRec dòng unique từ sheet '$SHEET_NAME'" -ForegroundColor Green
