$found = Get-ChildItem -Path "." -Filter "*Foaming Plan.xlsx" | Select-Object -First 1
if (-not $found) {
    Write-Host "Error: Excel file *Foaming Plan.xlsx not found."
    exit 1
}
$file = $found.FullName
...
$tempFile = Join-Path $env:TEMP ("foamingPlan_temp.xlsx")
try {
