# ============================================================
#  Sync W20-2026 L2  →  Supabase production_plan  (UPSERT FIX)
# ============================================================
...
try {
    ...
    # ── Tìm đúng tab W20-L2 (không phải "phat sinh") ─────────
    $sheet     = $null
    $sheetName = ""
    foreach ($s in $workbook.Sheets) {
        if ($s.Name -match "W20" -and $s.Name -match "L2" -and $s.Name -notmatch "phat\s*sinh") {
            $sheet     = $s
            $sheetName = $s.Name
        }
    }
    ...
    if ($null -eq $sheet) {
        Write-Host "[ERROR] Không tìm thấy sheet W20-L2!"
