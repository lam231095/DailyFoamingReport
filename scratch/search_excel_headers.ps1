...
$searchTerms = @("Người nhập", "Ngày Báo Cáo")
...
foreach ($file in $excelFiles) {
    ...
    Write-Host "Searching headers in $($file.FullName)..."
    try {
        ...
        foreach ($ws in $wb.Worksheets) {
            foreach ($term in $searchTerms) {
                ...
                if ($foundRange) {
                    Write-Host "FOUND HEADER '$term' in file '$($file.Name)', sheet '$($ws.Name)' at cell $($foundRange.AddressLocal)" -ForegroundColor Green
                }
            }
        }
        $wb.Close($false)
